...
% Make percentage be below 100% if smaller than original, greater than 100%
% if larger.
...
% Need check to make sure that, in cases where there are lots of cells that
...
% base_data.AllIC = create_AllICmask(base_IC); % Old Version

AllIC_base = base_data.AllIC;

% reg_data.AllIC = create_AllICmask(reg_data.GoodICf); % Old Version
...
if exist('RegistrationInfo.mat','file') == 2
    load('RegistrationInfo.mat');
...
if strcmpi(runthrough,'y')
    for j = 1:num_reg_ICs
        ...
        figure(FigNum)
        imagesc(base_data.AllIC + 3*reg_GoodICf{j});
        h = colorbar('YTick',[0 1 2 3 4],'YTickLabel',...
...
for j = 1:size(GoodICf_comb,2)
   ...
   elseif ~isempty(cell_map{j,reg_col}) && ~isempty(cell_map{j,reg_col-1}) % Combines neuron outline from both sessions! should we just keep original?
