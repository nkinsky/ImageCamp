...
figure(21);hist(ICsize,20);pause;

% Add up the IC filters
All_Mask = zeros(size(IC{1}));
for i = 1:NumICA
    All_Mask = All_Mask+FixedIC{i};
end
...
for i = 1:NumICA
  ...
  for k = 1:Xdim
      for l = 1:Ydim
         rmask{i}(k,l) = (norm([k l]-c) < LocalRad);
      end
  end  
...
for j = 1:NumFrames
  ...
  display(['Calculating F traces for movie frame ',int2str(j),' out of ',int2str(NumFrames)]);
    ...
    for i = 1:NumICA
      ...
      BGr(i,j) = sum(sum(tempFrame(tidx{i})));
    end
end
...
for i = 1:NumICA
    meanratio = mean(FLData(i,:))/mean(BGr(i,:));
    CFLData(i,:) = FLData(i,:)-BGr(i,:)*meanratio;
    CFLData(i,:) = zscore(detrend(NP_QuickFilt(CFLData(i,:),0.001,4,SR)));
end
...
for i = 1:NumICA
    ...
    subplot(2,1,1);imagesc(FixedIC{i});axis square;caxis([0 15]);
...
save FLdata.mat GoodFLData GoodCom GoodIC 
