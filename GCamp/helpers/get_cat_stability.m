function [ stay_prop, coactive_prop] = get_cat_stability(categories, neuron_map,...
    cat_designations)
% [stay_prop, coactive_prop,] = get_cat_stability(categories, neuron_map, ...
%       cat_designations)
%   Gets 2 cell stability metrics: coactivity probability, and probability of
%   staying in the same category for neurons in each category designated in 
...
for j = 1:length(cat_designations)
    ...
    num_stay = sum(category_array(:,1) == category_array(:,2) ...
        & category_array(:,2) == cat_use);
