function [ centroid, centroid_dist, axisratio, ratio_diff, ...
    orientation, orientation_diff, avg_corr, centroid_angle ] = ...
    dist_bw_reg_sessions( ROI_reg, varargin)
...
p.addParameter('orient_only', false, @islogical); % calculate orientation only to save time.
...
elseif orient_only
    for j = 1:num_sessions
        for k = 1:num_neurons
            stats_temp = regionprops(ROI_reg{j}{k},'Orientation');
            if length(stats_temp) ~= 1
