...
day = 3; % Day to analyze
movie_type = 'ChangeMovie'; % flag to ID if you want to look at ICmovie data or ChangeMovie data
rot_overwrite = 1; % 1 if you wish to analyze data for which a rotation has been intentionally NOT applied
...
sesh1list{3} = 'C:\Users\Nat\Documents\BU\Imaging\Working\GCamp Mice\G30\2env\11_21_2014\1 - 2env octagon mid 201B\Working\';
sesh2list{3} = 'C:\Users\Nat\Documents\BU\Imaging\Working\GCamp Mice\G30\2env\11_21_2014\2 - 2env octagon left 90CW 201B\Working\';
...
image_xpix = size(sesh1.AvgFrame_DF{1},2);
image_ypix = size(sesh1.AvgFrame_DF{1},1);
...
exclude = zeros(size(sesh1.AvgFrame_DF{1}));
...
corr_1_2 = corr_bw_sessions(sesh1.AvgFrame_DF,sesh2.AvgFrame_DF,exclude);
corr_1_win = corr_bw_sessions(sesh1.AvgFrame_DF_1st,sesh1.AvgFrame_DF_2nd,exclude);
corr_2_win = corr_bw_sessions(sesh2.AvgFrame_DF_1st,sesh2.AvgFrame_DF_2nd,exclude);
...
[num_rows, num_cols] = size(sesh1.AvgFrame_DF);
...
for k = 1:num_shuffles
    temp1 = cell(size(sesh1.AvgFrame_DF));
     ...
     for i = 1:num_rows
        ...
        for j = 1:num_cols
            temp1_1{i,j} = sesh1.AvgFrame_DF{i,shuffle1a(j)};
            temp1_2{i,j} = sesh2.AvgFrame_DF{i,shuffle1b(j)};
        end
    end
    
    corr_1_2_rowshuffle = corr_bw_sessions(temp1_1,sesh2.AvgFrame_DF,exclude); % Shuffle only sesh1
...
for k = 1:num_shuffles
   temp2_1 = cell(size(sesh1.AvgFrame_DF)); 
   temp2_2 = cell(size(sesh1.AvgFrame_DF));
   ...
   temp2_1(1:num_fields) = sesh1.AvgFrame_DF(shuffle2a);
   temp2_2(1:num_fields) = sesh2.AvgFrame_DF(shuffle2b);
