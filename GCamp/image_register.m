...
% base_file and register_file are the images to align, and should be the
% minimum projection of the motion corrected video from Mosaic.
% cell_map and cell_map_header are any cell mapping files from previously
% run sessions

% To do:
% 1) Make first column ALL ICs from base image - eventually this will be
% changed to be only all the ICs that look like cells (i.e. either
% acceptable ICs or noisy ICs, but excluding artifactual ICs)
% 2) Make second column the mapping from the base image acceptable cells to
% all the base image cells
% 3) Make third, fourth,...,nth columns the mappings of GoodICf for that
% day onto original ICs from the all the previous columns
% 3a) Need to create a vector from cell_map that holds the most recent IC
% designations in it
% 3b) Need to create a cell array that puts the UNION (OR) of all previous
% ICs (i.e. each IC can only grow from day to day)
