function [ ] = reverse_placefield2(cmperbin, speed_thresh)
...
if run_lens_correction == 1
    ...
    [ x, y ] = tracking_to_cm_std(x_in, y_in, pos_corr(ind_use).lm_x,...
        pos_corr(ind_use).lm_y, Pix2Cm, 'cm', limits_percent);

elseif run_lens_correction == 0
    x = pos_align.x;
    y = pos_align.y;
    disp(['Position data aligned to first session with a limits_percent of' ...
        num2str(pos_align.limits_percent) '.'])
    disp('Confirm this is the same used for session 1 and type "return" to continue');
...
save reverse_placefields.mat AvgFrame F0 AvgFrame_DF AvgFrame_1st ...
    AvgFrame_DF_1st AvgFrame_2nd AvgFrame_DF_2nd Occmap cmperbin Xedges ...
    Yedges speed_thresh x y t limits_percent
