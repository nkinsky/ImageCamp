...
for ll = 1:2
    for i = 1:4
        ...
        mega_corr((ll-1)*4+i).anaylsis_type = analysis_type{ll};
...
for m = 1:8
    ...
    mega_corr(m).day5_before = mega_corr(m).corr_matrix(5,1:4); % 3) day 5 to days 1-4
    mega_corr(m).day6_before = mega_corr(m).corr_matrix(6,1:4); % 4) day 6 to days 1-4
    mega_corr(m).day6_after = mega_corr(m).corr_matrix(6,7:8); % 5) day 6 to days 7-8
    temp = mega_corr(m).corr_matrix(1:4,7:8);
    mega_corr(m).before_after = temp(:); % 6) days 1-4 to days 7-8
end
