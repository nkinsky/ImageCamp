function [h, delta_angle_med, arena_rot, pshuf, ncells, coh_ratio ] = ...
    plot_pfangle_batch(sessions, sessions2, nshuf, PCfilter, coh_ang_thresh,...
    plot_flag, bin_size)
% [h, delta_angle_med, arena_rot, ncells, coh_ratio] = ...
%   plot_pfangle_batch( sessions, session2, nshuf, PCfilter, coh_ang_thresh )
        ...
        for k = j+1:num_sessions
            ...
            try
                [~, delta_angle_med(j,k), arena_rot(j,k), pshuf(j,k), ncells(j,k), ...
                    coh_ratio(j,k)] = plot_delta_angle_hist(...
