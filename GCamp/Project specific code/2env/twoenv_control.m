...
for j = 1:length(session)
    ...
    load(fullfile(session(j).Location,'PlaceMaps.mat'),'TMap_half','pval','pval_half','FT');
    session(j).TMap_half = TMap_half;
    session(j).pval = pval;
    session(j).pval_half = pval_half;
    session(j).numframes = size(FT,2);
    load(fullfile(session(j).Location,'PlaceMaps_oddeven.mat'),'TMap_half','pval','pval_half');
...
for j = 1:length(session)
    [PVtemp, ~] = get_PV_and_corr(session(j),0,'calc_half',...
        ...
        'alt_file_use','PlaceMaps.mat','PlaceMaps.mat');
...
save control_time_datav1 session
