...
for j = 1:max_day_lag
    [ stay_prop, coactive_prop, cat_names, ncoactive_all ] = ...
...
%% 
elim_outliers = false; % Get rid of sketchy zero and 1 points - need to investigate these! All from G31?
...
[~,~, hs_sp] = scatterBox(coactive_prop_all(~outlier_bool,1), ...
    grps_all(~outlier_bool,1), 'xLabels', ...
...
[~, ~, hs_apc] = scatterBox(coactive_prop_all(~outlier_bool,3), ...
    grps_all(~outlier_bool,1), 'xLabels', ...
    arrayfun(@num2str, 0:max_day_lag, 'UniformOutput', false), ...
...
prks = arrayfun(@(a) ranksum(coactive_prop_all(a == grps_all(:,1) & ~outlier_bool,1), ...
    coactive_prop_all(a == grps_all(:,1) & ~outlier_bool,3)),1:max_day_lag);
psign = arrayfun(@(a) signtest(coactive_prop_all(a == grps_all(:,1) & ~outlier_bool,1), ...
    coactive_prop_all(a == grps_all(:,1) & ~outlier_bool,3)),1:max_day_lag);
