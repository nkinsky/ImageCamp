...
cm = colormap('jet');
neurons_plot = [17 18 19 20 23 25 26 27 28 31 36 39 42 44 47 54 55 56];
...
%         xlabel(['Number hits = ' num2str(max(trials(k).PFnumhits(j,:)))])
    ...
    plot(1:NumFrames,trace_use(neurons_plot(j),:),'b-',find(FT(neurons_plot(j),:) & ~isrunning_all),trace_use(neurons_plot(j),FT(neurons_plot(j),:) & ~isrunning_all),'r.',...
        find(FT(neurons_plot(j),:) & isrunning_all & on_maze_log),trace_use(neurons_plot(j),FT(neurons_plot(j),:) & isrunning_all & on_maze_log),'g.');
