...
for i = 1:NumICA
    ...
    if (isempty(activations) == 0)
        ...
        for j = 1:size(activations,1)
            ...
            for k = frames
                if (k <= size(FT,2))
%                     tempFrame = h5read('FLmovie.h5','/Object',[1 1 k 1],[Xdim Ydim 1 1]);
                    tempFrame = h5read('ChangeMovie.h5','/Object',[1 1 k 1],[Xdim Ydim 1 1]);
                end
%                 imagesc(tempFrame);set(gca,'YDir','normal');hold on;plot(x{i},y{i},'-r');colormap(gray);title(int2str(j));caxis([0 0.35]);max(tempFrame(:));
                imagesc(tempFrame);set(gca,'YDir','normal');hold on;plot(x{i},y{i},'-r');colormap(gray);title(int2str(j));max(tempFrame(:)); % NRK Add
