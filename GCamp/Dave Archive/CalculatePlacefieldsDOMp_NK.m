...
[x,y,t,speed] = GetSpeed_NK(SR,Pix2Cm);
% x and y come out of this in cm units, speed is cm/sec, t is in sec
frame_start_image = min(t)*SR;
frame_end_image = max(t)*SR;
...
if (length(speed) <= Flength)
    % Chop the FL
    FT  = FT(:,frame_start_image:frame_end_image);
    Flength = length(speed)
else
    disp('Cineplex file is longer than Inscopix.  Did you screw something up?')
    keyboard
...
for i = 1:Flength
   MovMap(Xbin(i),Ybin(i)) = MovMap(Xbin(i),Ybin(i))+1; % number of time bins in each pixel
   if (i ~= Flength)
     SpeedMap(Xbin(i),Ybin(i)) = SpeedMap(Xbin(i),Ybin(i))+speed(i);
   end
   OccMap(Xbin(i),Ybin(i)) = OccMap(Xbin(i),Ybin(i))+1; % number of time bins in each pixel
end
SpeedMap = SpeedMap./OccMap; % NK - is this correct? or should this be multiplied by SR?
...
save PlaceMaps.mat x y t xOut yOut speed minspeed FT SP FMap TMap MovMap OccMap ip IC ;
