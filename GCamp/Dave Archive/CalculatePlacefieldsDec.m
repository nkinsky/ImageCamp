...
% NRK Edit start
if exist('Video.DVT','file')
    disp('Using Video.DVT file')
    [x,y,start_time,MoMtime] = PreProcessMousePosition('Video.DVT');
    
else
    DVT_filepath = ls('*.dvt');
    disp([ 'Using ' DVT_filepath ' file'])
    [x,y,start_time,MoMtime] = PreProcessMousePosition(DVT_filepath);
...
save PlaceMaps.mat x y t xOut yOut speed minspeed FT SP TMap MovMap SpeedMap OccMap Pix2Cm;
return;
