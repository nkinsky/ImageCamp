...
MaxDist = 50; % maximum radius from middle of ICA
LocalRad = 50;
MinRad = 20;
if (nargin == 0)
    MinSize = 50;
...
% STEP 0: Select folder where ICs and movie reside
...
for i = 1:NumICA
    % just zero out everything less RingThresh (NRK Edit)
    temp = IC{i};
    temp(temp < RingThresh) = 0; % NRK Edit
...
sAll_Mask = zeros(size(IC{1}));
for i = 1:NumICA
    temp = IC{i};
    temp(find(temp < RingThresh)) = 0; % NRK Edit
%     temp(find(temp < 25)) = 0;
    All_Mask = All_Mask+temp;
    
    
end
figure(11);imagesc(All_Mask);caxis([0 30]);colorbar;
...
tempData = importdata('FLmovie.mat'); %NRK Edit
% tempData = h5read('FLmovie.h5','/Object',[1 1 1 1],[1 1 Inf 1]); % NRK note - load .mat file instead - numFrames is there, and much quicker!
disp('determined number of frames');
% NumFrames = length(tempData); % 6/30 G20 square: 18048, 6/30 G20 triangle: 18063
NumFrames = tempData.Object.DataSize(3); % NRK Edit
t = (1:NumFrames)/SR;
...
for j = 1:NumFrames
    tempIn = h5read('FLmovie.h5','/Object',[1 1 j 1],[Xdim Ydim 1 1]);
...
for i = 1:NumICA
    ...
    [counts,centers] = hist(CFLData(i,:),400);
    [val,idx] = max(counts);
    CFLData(i,:) = CFLData(i,:)-centers(idx);
...
save MagicNumbers.mat NumICA RingThresh SR MaxDist LocalRad MinRad MinSize; % NRK Edit
