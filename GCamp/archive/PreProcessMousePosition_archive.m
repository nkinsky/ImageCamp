...
while (strcmp(MorePoints,'y'))
  ...
  if (strcmp(MorePoints,'n') ~= 1 && strcmp(MorePoints,'g') ~= 1)
    ...
    for i = 1:floor(length(framesToCorrect)/2);
        ...
        figure(555);
        subplot(4,3,1:3);plot(time,Xpix);xlabel('time (sec)');ylabel('x position (cm)');hold on;ylabel('x position (cm)');yl = get(gca,'YLim');line([MoMtime MoMtime], [yl(1) yl(2)],'Color','r');hold off;axis tight;set(gca,'XLim',[sFrame/aviSR eFrame/aviSR]);
        subplot(4,3,4:6);plot(time,Ypix);xlabel('time (sec)');ylabel('y position (cm)');hold on;ylabel('x position (cm)');yl = get(gca,'YLim');line([MoMtime MoMtime], [yl(1) yl(2)],'Color','r');hold off;axis tight;set(gca,'XLim',[sFrame/aviSR eFrame/aviSR]);
