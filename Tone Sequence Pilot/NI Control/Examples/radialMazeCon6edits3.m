function radialMazeCon6edits3    %plays tone randomly during ITI after 30 regular trials.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
tone_ts=nan(size(trials));    %denotes when tone was played
...
start(tim);%start task
            ...
            if size(joyevents,1)>0  && starts~=0
                switch joyevents(1)
                    ...
                    case 5
                        ...
                        if tr(r)> 30 && rand(1)> 0.5 && delay
                            set delay1on = true;
                            set delaytime1= randi(9);                                                   
                            playTone  %plays tone for at the choice point for rewarded arms
    ...
    function filename = saveFile(dname,choice)
        ...
        if starts~=0
            ...
            duration=toc(starts); %length of session
            ...
            if strcmp(choice,'No')
                ...
                save(filename,'ratname','day','when','trials','arms','doorts','waterts',...
                    'maxoutts','centerts','startstop','duration','type','ITI','endarm','rewArms',...
                    'numtr','trial2crit','rewardts', 'tonets'); %save data
