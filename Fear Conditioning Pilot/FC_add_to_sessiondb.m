...
% Import session database to add files to...
...
if db_filename == 0
    ...
    testgroup = importdata([db_folder db_filename]);
    savepath = [db_folder db_filename(1:end-4) '_archive_' datestr(now,30) '.mat'];
    save(savepath,'testgroup')
    % Archive existing database file
end

% testgroup(1).name = 'FC3';
% testgroup(2).name = 'FC4';
cd(db_folder);
[filename, folder] = uigetfile('*.dvt', ...
...
lengths = cellfun(@(a) diff(a),break_indices,'UniformOutput',0); % Get lenghts of text between underscores
...
elseif ~isempty(db_filename)
    save([db_folder db_filename],'testgroup')
end
cd(start_dir);
